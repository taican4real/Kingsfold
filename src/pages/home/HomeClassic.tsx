  ...
  return (
    <div className="flex flex-col w-full bg-[#fdfcf9] font-serif">
      ...
      <section className="relative h-[90vh] flex items-center justify-center text-center p-4">
        ...
        <div className="relative z-20 max-w-3xl text-white border-2 border-white/20 p-12 backdrop-blur-sm pointer-events-none">
          ...
          >
            <div className="w-20 h-24 mx-auto mb-8 bg-white/10 flex items-center justify-center border border-white/30">
              <span className="text-4xl">K</span>
            </div>
            <h1 className="text-4xl md:text-6xl mb-6 tracking-tight leading-tight">
              Honoring Tradition,<br />Inspiring Innovation.
            </h1>
            <p className="text-lg font-sans font-light tracking-wide opacity-80 mb-10 italic">
              Established on principles of discipline and distinction.
            </p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <Link to="/about" className="px-8 py-4 bg-white text-wine font-sans font-bold uppercase tracking-widest text-[10px] hover:bg-cream-light transition-all">
                The Heritage
              </Link>
              <Link to="/admissions" className="px-8 py-4 border border-white text-white font-sans font-bold uppercase tracking-widest text-[10px] hover:bg-white hover:text-wine transition-all">
                Admissions Call
              </Link>
            </div>
          </motion.div>
      ...
      <section className="py-32 container mx-auto px-4 max-w-6xl">
        <div className="text-center mb-24">
          <span className="text-red uppercase tracking-[0.4em] text-[10px] font-bold mb-4 block underline underline-offset-8">Mark of Distinction</span>
          <h2 className="text-5xl text-wine-dark mt-8 leading-tight">A Pedigree of Excellence</h2>
        </div>
        
        <div className="grid grid-cols-1 md:grid-cols-3 gap-16">
          {[
            { icon: Award, title: "Grand Honors", desc: "Consistently ranked among the top 1% of international schools in West Africa." },
            { icon: ScrollText, title: "British Standards", desc: "Accredited by leading international bodies for superior academic curricula." },
            { icon: BookOpen, title: "Liberal Arts", desc: "A holistic approach that cherishes literature, history, and classical logic." }
          ].map((item, i) => (
            <div key={i} className="flex flex-col items-center text-center">
              <div className="w-16 h-16 rounded-full border border-wine/10 flex items-center justify-center text-wine mb-8">
                <item.icon size={28} />
              </div>
              <h3 className="text-2xl mb-4 text-wine-dark">{item.title}</h3>
